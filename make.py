...
SERVICE = [
    ...
    ("/service-area/", "service-area.html",
     "המרת קלטות באזור רחובות, נס ציונה וראשון לציון | STUDIOLAB",
     "STUDIOLAB ברחובות משרת את כל אזור השפלה — נס ציונה, ראשון לציון, יבנה, "
     "גדרה ומזכרת בתיה. מרחקים, זמני הגעה ואיסוף."),
]
...
<p><strong>שימו לב גם למכניקה.</strong> קלטת שהסרט בה נמתח, מקומט או שהתחיל להידבק לעצמו אחרי שנים ללא הרצה &#8212; עלולה להיתפס בזמן הניגון. קלטת כזו צריכה בדיקה וטיפול לפני שמנגנים אותה, לא אחריו, כדי לא לפגוע בסרט או במכשיר.</p>
<p>קלטת שנראית נקייה, מסתובבת חופשי והסרט בה שלם &#8212; כנראה בסדר.</p>
